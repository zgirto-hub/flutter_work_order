...
- [ ] T001 Verify current branch is `088-rag-refusal-diagnostic` and `backend/tests/test_rag_quality.py` on the branch matches the 2026-04-19 baseline (rewritten questions, 87 total). If the branch or test file is wrong, stop and realign with `main` before proceeding.
...
- [ ] T010 [US1] Add `GET /api/admin/rag-diagnostics/{id}` to the same router. Returns the full row per contracts/diagnostic-api.md §3 (all 11 columns including the JSONB payloads). `404` if id not found.
- [ ] T011 [P] [US1] Wire `admin_rag_diagnostics` router into the FastAPI app in `backend/main.py` (or whichever file includes routers — match the existing pattern for other admin routers). Prefix `/api/admin/rag-diagnostics`.
