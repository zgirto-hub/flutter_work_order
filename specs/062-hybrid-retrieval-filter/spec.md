...
- Q: Do follow-up turns in a multi-turn conversation stay scoped? → A: Yes. If the original question has no system keyword but the history-aware query rewriter (spec 042) produces a rewritten query that does, detection re-runs on the rewrite. So "how to restart CADAS-ATS?" → "any other steps?" stays scoped to CADAS-ATS on the follow-up. Implementation: `detect_system(question)` runs first; if None and `search_query != question`, runs `detect_system(search_query)` as a second pass.
