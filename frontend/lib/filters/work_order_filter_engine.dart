...
class WorkOrderFilterEngine {
  static List<WorkOrder> applyFilters(
    ...
    if (filter.statusFilter != "All") {
      filtered =
          filtered.where((wo) => wo.status == filter.statusFilter).toList();
    }
    ...
    if (filter.searchQuery.isNotEmpty) {
      ...
      filtered = filtered.where((wo) {
        final jobNoMatch = wo.jobNo.toLowerCase().contains(query);

        final titleMatch = wo.title.toLowerCase().contains(query);

        final descriptionMatch = wo.description.toLowerCase().contains(query);
    ...
    if (filter.selectedEmployeeId != null) {
      filtered = filtered.where((wo) {
        return wo.assignedTechnician?.id == filter.selectedEmployeeId;
      }).toList();
    }
