...
class _AddWorkOrderScreenState extends State<AddWorkOrderScreen> {
  ...
  bool _savedSignatureLoading = false;
  ...
  void initState() {
    ...
    if (widget.workOrder != null) {
      ...
      if (widget.prefillStatus != null) {
        final allowedStatuses = ['Pending', 'In Progress', 'Closed', 'Open'];
        if (allowedStatuses.contains(widget.prefillStatus)) {
      ...
      if (widget.prefillDepartmentId != null &&
          widget.prefillDepartmentId!.isNotEmpty) {
      ...
      if (_userRole == 'admin') {
        ...
        if (currentUser != null &&
          ...
          if (_isTechnician && !departments.any((d) => d.id == userDeptId)) {
            final allDepts =
                await _departmentService.fetchDepartments(isActive: true);
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (role == 'reporter') {
          ...
          currentUser = await _userService.fetchCurrentUser();
        } else {
          currentUser = await _userService.fetchCurrentUser();
          if (role == 'technician') {
            _isTechnician = true;
            if (currentUser != null &&
                ...
                currentUser.departmentId!.isNotEmpty) {
              ...
              if (deptName.isEmpty) {
                try {
                  final allDepts =
                      await _departmentService.fetchDepartments(isActive: true);
                  final match =
                      allDepts.where((d) => d.id == deptId).firstOrNull;
              ...
              if (mounted) {
                ...
                _loadEmployees(
                    departmentId: widget.workOrder?.departmentId ?? deptId);
              }
            }
          }
        }
        ...
        if (currentUser != null) {
          ...
          if (userId.isNotEmpty) {
            ...
            await _loadSavedSignature(userId: userId);
          }
        }
      } else {
        if (!mounted) return;
  ...
  Future<void> _loadUserProfile(String email) async {
    ...
    try {
      final res = await http.get(
        Uri.parse(
            '${AppConfig.baseUrl}/users/me?email=${Uri.encodeComponent(email)}'),
      );
  ...
  Map<String, dynamic> _formSnapshot() {
    return {
      ...
      'type': selectedType,
      'assigned_technician_ids': List<String>.from(_selectedEmployeeIds)
        ..sort(),
    };
  }
  ...
  void _onFormFieldChanged() {
    ...
    _autoSaveTimer?.cancel();
    _autoSaveTimer =
        Timer(const Duration(milliseconds: 1500), _performAutoSave);
  }
        ...
        if (showStatus) ...[
          if (_autoSaveStatus == AutoSaveStatus.saving)
            ...
            Icon(Icons.cloud_done_outlined,
                size: 14, color: AppColors.closedText)
  ...
  Future<void> _loadEmployees({String? departmentId}) async {
    final deptId = departmentId ??
        (selectedDepartmentId.isNotEmpty ? selectedDepartmentId : null);
  ...
  Future<void> _loadSignatures() async {
    ...
    try {
      final sigs =
          await _signatureService.fetchSignatures(widget.workOrder!.id);
      if (mounted) setState(() => _signatures = sigs);
    } catch (_) {}
    if (mounted) setState(() => _signaturesLoading = false);
  }

  Future<void> _loadSavedSignature({String? userId}) async {
    final targetId = userId ?? _currentUserId;
    if (targetId == null || targetId.isEmpty) return;
    setState(() => _savedSignatureLoading = true);
    try {
      final path = await _signatureService.fetchUserSignature(targetId);
      if (mounted) setState(() => _savedSignaturePath = path);
    } catch (_) {}
    if (mounted) setState(() => _savedSignatureLoading = false);
  }
    ...
    try {
      ...
      final commentBody = text.isNotEmpty
          ...
          : (uploadedFileName != null
              ? 'uploaded a file: $uploadedFileName'
              : '');
                    ...
                    children: [
                      GestureDetector(
                        onTap: () async {
                          ...
                          if (!mounted) return;
                          navigator.pop(_hasAutoSaved ? 'updated' : null);
                        },
                        child: Container(
                          width: 34,
                          height: 34,
  ...
  Widget _buildDetailsTab() {
    ...
    final extraBottom = _fieldHasFocus && bottomInset == 0 && isMobile
        ...
        child: Column(
          ...
          children: [
            ...
            if (widget.workOrder != null && _userRole == 'admin') ...[
              ...
              SizedBox(height: 10),
              TextFormField(
                initialValue: widget.workOrder!.createdByName ??
                    widget.workOrder!.createdByEmail,
                readOnly: true,
                decoration: InputDecoration(
                  labelText: "Created By",
            ...
            DropdownButtonFormField<String>(
              ...
              items: _departments.isEmpty
                  ...
                  : _departments
                      ...
                      .toList(),
              onChanged: (canEdit && !_isTechnician)
                  ? (v) {
                      if (v != null) {
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Center(
              child: _buildSignatureImage(sig.signaturePath),
            ),
          ),
        ],
      ),
  ...
  Future<void> _openSignatureCanvas(String role) async {
    ...
    await _submitSignature(role: role, signatureData: base64Png);
  }
  ...
  Future<void> _submitSignature({
    ...
    if (useSaved &&
        (_savedSignaturePath == null || _savedSignaturePath!.isEmpty)) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('No saved signature available'),
    ...
    try {
      await _signatureService.saveSignature(
        ...
        signerEmail: email,
        signerRole: role,
        signatureData: useSaved ? null : signatureData,
        useSaved: useSaved,
      );
  ...
  Future<void> _approveAndSign(WorkOrderSignature techSig,
      {required bool useSaved}) async {
    if (!mounted) return;
    String? base64Png;

    if (useSaved) {
      if (_savedSignaturePath == null || _savedSignaturePath!.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('No saved signature available'),
              backgroundColor: Colors.red),
      ...
      base64Png = await SignatureCanvas.show(context);
      if (base64Png == null || !mounted) return;
    }
  ...
  Widget _buildSignatureActionCard({
    ...
    if (hasSaved) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _savedSignaturePreviewCard(),
          SizedBox(height: 8),
          ElevatedButton.icon(
            icon: Icon(Icons.check, size: 16),
            label: Text('Use Saved Signature'),
            onPressed: () => _submitSignature(role: role, useSaved: true),
            style: ElevatedButton.styleFrom(
              backgroundColor: AppColors.accent,
          ...
          TextButton(
            onPressed: () => _openSignatureCanvas(role),
            child: Text('Draw New Instead'),
          ),
        ],
      );
    }
  ...
  Widget _buildAdminApprovalActions(WorkOrderSignature techSig) {
    if (_savedSignatureLoading) {
      return Center(
        child:
            CircularProgressIndicator(color: AppColors.accent, strokeWidth: 2),
    ...
    if (hasSaved) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _savedSignaturePreviewCard(title: 'Your Saved Signature'),
          SizedBox(height: 8),
          ElevatedButton.icon(
            icon: Icon(Icons.check, size: 16),
            label: Text('Use Saved Signature'),
            onPressed: () => _approveAndSign(techSig, useSaved: true),
            style: ElevatedButton.styleFrom(
          ...
          TextButton(
            onPressed: () => _approveAndSign(techSig, useSaved: false),
            child: Text('Draw New Instead'),
          ),
        ],
      );
  ...
  Widget _savedSignaturePreviewCard({String title = 'Saved Signature'}) {
    final path = _savedSignaturePath;
    if (path == null || path.isEmpty) return SizedBox.shrink();
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Center(child: _buildSignatureImage(path)),
          ),
        ],
      ),
    );
  ...
  Widget _buildComposeBar() {
    ...
    final bottomInset = MediaQuery.of(context).viewInsets.bottom;
    final isMobile = MediaQuery.of(context).size.width < 600;
    final extraBottom = _commentHasFocus && bottomInset == 0 && isMobile
        ? MediaQuery.of(context).size.height * 0.45
        : bottomInset;
          ...
          children: [
            ...
            if (badge > 0) ...[
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: active ? AppColors.accent : AppColors.bgSurface3,
                  borderRadius: BorderRadius.circular(8),
                ),
                child: Text(
                  '$badge',
