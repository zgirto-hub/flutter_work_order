...
- Dart 3.x / Flutter 3.x (frontend); Python 3.10 (backend) + FastAPI, Supabase Python client (backend, existing); `http`, `supabase_flutter`, Flutter Material (frontend, existing). **No new dependencies.** (093-delete-user)
- Supabase (PostgreSQL) — existing `users`, `user_activity_log` tables. No schema changes, no migrations. Supabase Auth (`auth.users`) via admin client. (093-delete-user)
- Python 3.10 (backend), Dart 3.x / Flutter 3.x (frontend) + FastAPI, Supabase Python client (backend, existing); `http`, `supabase_flutter`, Flutter Material, `file_picker` (frontend, existing). **No new dependencies.** (094-dept-auto-assign)
- Supabase (PostgreSQL) — existing `files`, `users`, `departments`, `technician_departments` tables. **No schema changes, no migrations.** Relies on the `files.department_id` column added in spec 092. (094-dept-auto-assign)
...
## Recent Changes
- 094-dept-auto-assign: Added Python 3.10 (backend), Dart 3.x / Flutter 3.x (frontend) + FastAPI, Supabase Python client (backend, existing); `http`, `supabase_flutter`, Flutter Material, `file_picker` (frontend, existing). **No new dependencies.**
- 093-delete-user: Added Dart 3.x / Flutter 3.x (frontend); Python 3.10 (backend) + FastAPI, Supabase Python client (backend, existing); `http`, `supabase_flutter`, Flutter Material (frontend, existing). **No new dependencies.**
